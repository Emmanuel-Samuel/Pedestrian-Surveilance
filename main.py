"""
    ...
    Date Revised: 14th April 2023
    Purpose: Pedestrian monitoring system using computer vision
"""
...
TEXT_POSITION_COUNT_PEOPLE = (10, 100)
TEXT_SIZE = 1.2
FONT = cv2.FONT_HERSHEY_SIMPLEX
SAVE_IMAGE = True
IMAGE_DIR = "./vehicles"
VIDEO_SOURCE = "cars_people.mp4"
VIDEO_OUT = "result_traffic.avi"
    ...
    while (cap.isOpened()):
        ...
        for cnt in contours:
            ...
            if area > minArea and area <= maxArea:
                ...
                cv2.putText(roi, 'People', (x, y-2), FONT, 0.5, (255,255,255), 1, cv2.LINE_AA)
                for i in objects:
                    if abs(x - i.getX()) <= w and abs(y - i.getY()) <= h:
                        ...
                        if i.going_DOWN(DOWN_limit) == True:
                            cnt_cars += 1
                            if SAVE_IMAGE:
                                save_frame(roi, IMAGE_DIR + '/people_DOWN_%04d.png' % frame_number)
        ...
        str_people = 'People: ' + str(cnt_cars)
        ...
        cv2.putText(frame, str_people, TEXT_POSITION_COUNT_PEOPLE, FONT, 1, (255,255,255), 3, cv2.LINE_AA)
        cv2.putText(frame, str_people, TEXT_POSITION_COUNT_PEOPLE, FONT, 1, (232, 162, 0), 2, cv2.LINE_AA)
